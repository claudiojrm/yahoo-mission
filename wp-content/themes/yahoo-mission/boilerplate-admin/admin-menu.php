...
/*	5)	Add Boilerplate options to page as requested */
		...
		?>
			<table class="form-table">
				<tr>
					<th><label for="imagem"><?php _e("Imagem perfil"); ?></label></th>
					<td>
						<img src="<?php echo esc_attr( $imagem ); ?>" style="border:1px solid #ccc;padding:1px;float:left;margin:0 10px 0 0;" width="80">
						<input type="text" name="imagem" id="imagem" value="<?php echo esc_attr( $imagem ); ?>" class="regular-text" /><br />
						<span class="description"><?php _e("Por favor insira o caminho da imagem do perfil."); ?></span>
					</td>
				</tr>
			</table>
		<?php }		
		add_action( 'personal_options_update', 'save_info_extras' );
		add_action( 'edit_user_profile_update', 'save_info_extras' );
		 
		function save_info_extras( $user_id ) {		 
			if ( !current_user_can( 'edit_user', $user_id ) ) { return false; }
			update_user_meta($user_id, 'imagem',			$_POST['imagem']);
		}
?>
